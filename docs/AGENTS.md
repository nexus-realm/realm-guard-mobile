...
- **What `flutter test` cannot reach** (VM Dart): SQLCipher (`PRAGMA key`), the Rust lib (FFI), and platform channels (`local_auth`, Keystore, `path_provider`). Everything else *is* host-testable.
- **Drift IS host-testable** — `AppDatabase.forTesting(NativeDatabase.memory())` gives the same schema and migrations without SQLCipher. Use it for repository, projector and store tests rather than faking the DB.
- **Secure storage is fakeable** — `FlutterSecureStorage.setMockInitialValues({})` (v10) makes `SecureCrdtDeviceIdStore` / `SecureWrappedVaultKeyStore` testable.
- `Uint8List ==` is **reference** equality in tests → index or compare via `UuidValue`, never `firstWhere(x == otherUint8List)`.
- Widget tests: `pumpAndSettle` **times out on infinite animations** (loading spinners) → drive with explicit `pump(Duration)`. `ViewTitle` renders `title.toUpperCase()`.
- Run focused security tests when touching crypto: `flutter test test/core/security/`.

### Coverage

- `dart run tool/coverage.dart` — runs the suite and prints an **honest** figure: generated code (`*.g.dart`, `lib/src/rust/`) stripped, and `test/all_imports_test.dart` (generated by `tool/gen_coverage_imports.dart`) forces every `lib/` file into the report so untested files count as 0 % instead of vanishing.
- **Out-of-scope list** in `tool/coverage.dart` (`_outOfScope`), one justification per file. Admission rule: a file goes there only if **no host test can execute it** (composition root, platform channel, SQLCipher, FFI pass-through, drift table DSL) — *never* because it is merely untested. Both figures are printed (testable perimeter + raw), and the excluded files are listed on every run.
- Current: **54.3 %** on the testable perimeter; `lib/core` at **92 %** (`core/sync` 100 %, `core/security` 92 %, `core/database` 79 %). Biggest remaining gaps: `home_tab.dart`, `sync_page.dart`, `unlock_page.dart`, the autofill view models, and the `app_database.dart` migration ladder (needs `drift_dev schema` helpers).
